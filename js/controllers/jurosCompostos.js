...
document.getElementById('jurosCompostos').addEventListener('keypress', function (event) {
    // Verifica se a tecla pressionada é 'Enter'
    if (event.key === 'Enter') {
        event.preventDefault(); // Impede o comportamento padrão de enviar o formulário
        calcularJurosCompostos(); // Chama a função de cálculo
        }
});
...
function calcularJurosCompostos() {
    ...
    const montante = capitalInicial *  Math.pow(1+ taxaJuros, numPeriodos);
    ...
    document.getElementById('juros').value = 'R$ ' + juros.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
